...
dimensions = [x.split('x') for x in puzzle_input.splitlines()]

def surface_area(length, width, height):
    return 2 * length * width + 2 * width * height + 2 * height * length

def surface_area_smallest_side(length, width, height):
...
def get_bow_length(length, width, height):
...
for present in dimensions:
    ...
    area = surface_area(*present)
    area += surface_area_smallest_side(*present)
    count += area

    ribbon = get_ribbon_length(*present)
    ribbon += get_bow_length(*present)
